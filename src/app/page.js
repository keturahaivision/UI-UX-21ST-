    ...
    <div className="bg-brand-coal">
      ...
      <section className="bg-brand-paper text-brand-ink">
        <div className="u-container py-28 md:py-32">
          ...
          <Reveal delay={80}>
            <p className="mt-6 max-w-4xl r-h text-[1.9rem] leading-[1.22] text-brand-ink md:text-[2.9rem]">
          ...
          <Reveal delay={160}>
            <p className="mt-8 max-w-2xl font-ptsans text-lg leading-relaxed text-brand-muted">
      ...
      {/* ── 03 · Expertise (warm light) ────────────── */}
      <section className="bg-brand-mist text-brand-ink">
        <div className="u-container py-24">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <Reveal><p className="r-eyebrow">What we do</p></Reveal>
              <Reveal delay={80}><h2 className="mt-4 r-h text-4xl text-brand-ink md:text-5xl">Where master planning meets infrastructure.</h2></Reveal>
              ...
              {disciplines.map(([k, v], i) => (
                <Reveal key={k} delay={i * 60}>
                  <Link href={`/projects?d=${encodeURIComponent(k)}`}
                    className="tile-light group flex h-full flex-col justify-between rounded-xl p-4 transition-all hover:-translate-y-0.5 hover:border-brand-red/50">
                    <span className="font-raleway text-[13px] font-bold text-brand-red">{String(v).padStart(2, '0')}</span>
                    <span className="mt-6 font-raleway text-sm font-semibold leading-tight text-brand-ink">{k}</span>
                  </Link>
                </Reveal>
              ))}
            </div>
      ...
      {/* ── 04 · Interactive regional map (cinematic dark) ── */}
      <section className="bg-brand-coal text-white">
        <div className="u-container py-24">
      ...
      {/* ── 05 · Selected work (warm light) ────────── */}
      <section className="bg-brand-paper text-brand-ink">
        <div className="u-container py-24">
          <div className="flex items-end justify-between">
            <div>
              <Reveal><p className="r-eyebrow">Selected work</p></Reveal>
              <Reveal delay={80}><h2 className="mt-3 r-h text-4xl text-brand-ink md:text-5xl">The developments DMF made possible.</h2></Reveal>
            </div>
                ...
                <Link href={`/projects/${p.slug}`} className="group block">
                  <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-brand-ink">
                    <img src={asset(p.thumb || p.hero)} alt={p.name} loading="lazy"
                      className="h-full w-full object-cover transition-transform duration-[700ms] ease-[cubic-bezier(.2,0,0,1)] group-hover:scale-[1.06]" />
                    <div className="absolute inset-0 bg-gradient-to-t from-brand-ink/60 via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                  </div>
                  <p className="mt-4 font-raleway text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-red">{p.disciplines[0]}</p>
                  <h3 className="mt-1 r-h text-xl text-brand-ink">{p.name}</h3>
                  {p.location && <p className="mt-0.5 font-ptsans text-sm text-brand-muted">{p.location}</p>}
                </Link>
              </Reveal>
            ))}
          </div>
      ...
      {/* ── 06 · Scale (cinematic dark, image-backed) ── */}
      <section className="relative overflow-hidden bg-brand-coal text-white">
        <img src={asset(scaleImg)} alt="" aria-hidden className="absolute inset-0 h-full w-full object-cover opacity-[0.18]" />
          ...
          <div className="mt-8 grid gap-3 md:grid-cols-4">
            {PROCESS.map(([t, d], i) => (
              <Reveal key={t} delay={i * 70}>
                <div className="tile-light h-full rounded-2xl p-6">
                  <span className="font-raleway text-2xl font-extrabold text-brand-red">{String(i + 1).padStart(2, '0')}</span>
                  <h3 className="mt-4 font-raleway text-lg font-bold text-brand-ink">{t}</h3>
                  <p className="mt-2 font-ptsans text-sm leading-relaxed text-brand-muted">{d}</p>
                </div>
              </Reveal>
            ))}
          </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              {data.clients.filter((c) => c.logo).map((c, i) => (
                <Reveal key={c.logo} delay={(i % 5) * 50}>
                  <div className="flex items-center justify-center rounded-xl border border-brand-line bg-white p-6">
                    <img src={asset(c.logo)} alt={c.name || 'DMF Engineering client'} loading="lazy"
                      className="max-h-11 w-auto opacity-70 grayscale transition hover:opacity-100 hover:grayscale-0" />
                  </div>
                </Reveal>
              ))}
            </div>
      ...
      {/* ── 09 · CTA (cinematic dark, image-backed) ── */}
      <section className="relative overflow-hidden bg-brand-coal text-white">
        <img src={asset(data.flagships[0].hero)} alt="" aria-hidden className="absolute inset-0 h-full w-full object-cover opacity-[0.22]" />
        ...
        <div className="u-container relative py-28 text-center">
          ...
          <Reveal delay={100}><p className="mx-auto mt-5 max-w-lg font-ptsans text-white/70">From master plan to movement, we take it from the ground up.</p></Reveal>
          <Reveal delay={180}>
