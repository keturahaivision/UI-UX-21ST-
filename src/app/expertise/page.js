  ...
  return (
    <>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-2 gap-y-3">
          {HOW_WE_WORK.map((step, i) => (
            <span key={step} className="flex items-center gap-2">
              <span className="rounded-full tile px-4 py-2 font-mono text-[11px] uppercase tracking-[0.14em] text-white/70">
                {String(i + 1).padStart(2, '0')} {step}
              </span>
              {i < HOW_WE_WORK.length - 1 && <span className="text-brand-red">→</span>}
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="u-container pb-32">
        <div className="grid gap-px overflow-hidden rounded-[1.25rem] bg-white/10 md:grid-cols-2">
          {services.map((s, i) => (
            <div key={s.slug} className="bg-brand-coal p-8 md:p-10">
              <div className="flex items-baseline justify-between gap-4">
                <div className="flex items-baseline gap-4">
                  <span className="font-mono text-[11px] tracking-[0.14em] text-brand-red">{String(i + 1).padStart(2, '0')}</span>
                  <h2 className="font-raleway text-2xl font-medium">{s.name}</h2>
                </div>
                {s.related.length > 0 && (
                  <span className="shrink-0 font-mono text-[11px] uppercase tracking-[0.14em] text-white/45">{String(s.related.length).padStart(2, '0')} projects</span>
                )}
              </div>
              <p className="mt-4 text-[15px] leading-relaxed text-white/70">{s.description}</p>
              {s.related.length > 0 && (
                <div className="mt-6 flex flex-wrap gap-2">
                  {s.related.slice(0, 4).map((p) => (
                    <Link key={p.slug} href={`/projects/${p.slug}`} className="rounded-full tile px-3 py-1.5 text-xs font-medium text-white/70 transition-colors hover:text-brand-red">{p.name}</Link>
                  ))}
                  {s.related.length > 4 && (
                    <Link href={`/projects?d=${encodeURIComponent(s.label)}`} className="px-3 py-1.5 text-xs font-semibold text-brand-red underline underline-offset-4">+{s.related.length - 4} more →</Link>
                  )}
                </div>
              )}
            </div>
