...
export default function ProjectGrid() {
  ...
  const Pill = ({ active, onClick, children, count }) => (
    <button onClick={onClick}
      className={`rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-colors duration-300 ${active ? 'border-sys-red bg-sys-red text-white' : 'border-sys-line bg-white/[0.04] text-sys-muted hover:border-sys-muted/50 hover:text-sys-ink'}`}>
      {children}{typeof count === 'number' && <span className="ml-1.5 opacity-60">{count}</span>}
    </button>
  );

  const Label = ({ children }) => (
    <span className="mr-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-sys-faint">{children}</span>
  );

  return (
    <div>
      <div className="flex flex-col gap-4 border-b border-sys-line pb-8">
      ...
      <h2 className="mt-6 font-mono text-[11px] uppercase tracking-[0.16em] text-sys-faint" aria-live="polite">{filtered.length} projects</h2>
      ...
      {filtered.length === 0 && <p className="mt-16 text-center text-sys-faint">No projects match this combination.</p>}
    </div>
  );
}
