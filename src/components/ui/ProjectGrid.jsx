...
export default function ProjectGrid() {
  ...
  const Pill = ({ active, onClick, children, count }) => (
    <button onClick={onClick}
      className={`rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-colors duration-300 ${active ? 'border-brand-red bg-brand-red text-white' : 'border-white/10 bg-white/[0.04] text-white/70 hover:border-white/40 hover:text-white'}`}>
      {children}{typeof count === 'number' && <span className="ml-1.5 opacity-60">{count}</span>}
    </button>
  );

  const Label = ({ children }) => (
    <span className="mr-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-white/45">{children}</span>
  );
  ...
  return (
    <div>
      ...
      <h2 className="mt-6 font-mono text-[11px] uppercase tracking-[0.16em] text-white/45" aria-live="polite">{filtered.length} projects</h2>
      ...
      {filtered.length === 0 && <p className="mt-16 text-center text-white/45">No projects match this combination.</p>}
    </div>
  );
}
